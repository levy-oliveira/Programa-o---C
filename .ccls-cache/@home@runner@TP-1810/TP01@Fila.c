...
Fila *fila_cria(){
  ...
  fila->fila_aluno = (Aluno*) malloc(c_começo * sizeof(Aluno*));
  fila->c_maxima = c_começo;
  fila->tamanho = 0;
  return fila;
}
...
int fila_insere(Fila *fila, Aluno *aluno){
  ...
  alu_acessa(aluno, &matricula_base, nome_base, curso_base);

  if (fila->tamanho > 0){
    for (int i=0; i < fila->tamanho; i++){
      alu_acessa((&(fila->fila_aluno))[i], &matricula, nome, curso);
      if (matricula == matricula_base){
        return 0;
      }
    }
  }
  
  if (fila->tamanho <= fila->c_maxima) {
    int tamanho_anterior = fila->tamanho;
    fila->tamanho++;
    (&(fila->fila_aluno))[tamanho_anterior] = aluno;
    return 1;
  }
  return 2;
}
...
Aluno *fila_retira(Fila *fila) {
  ...
  Aluno *aluno_removido = (&(fila->fila_aluno))[0];
  
  for(int i = 1; i < fila->tamanho; i++){
    (&(fila->fila_aluno))[i - 1] = (&(fila->fila_aluno))[i];
  }

  (&(fila->fila_aluno))[fila -> tamanho + 1] = NULL;
...
Aluno *fila_busca(Fila *fila, int matricula){
  ...
  for (int i = 0; i < fila->tamanho; i++) {
    ...
    alu_acessa((&(fila->fila_aluno))[i], &matric, nome, curso);
    
    if (matricula == matric) {
      return (&(fila->fila_aluno))[i];
    }
  }
  return NULL;
}
